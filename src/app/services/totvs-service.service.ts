...
export class TotvsService {
  ...
  obterColunasReparos(): Array<PoTableColumn> { //colunasDetalhe
    return [
      ...
      { property: 'NFRecebe',      label: 'NF',         visible: true },
      { property: 'DataDevol',     label: 'Dt.Saida',   visible: true },
      { property: 'RequisSaida',   label: 'Requisição', visible: true },
      ...
      { property: 'NTecnico',      label: 'Num.Tec',    visible: true },
      { property: 'Situacao',      label: 'Situação',   visible: true },
      { property: 'Observacao',    label: 'Obs',        visible: true },    
    ]
  }
  ...
  public ObterDadosNota(params?: any){
    return this.http.post(`${this._url}/ObterDadosNota`, params, { headers:headersTotvs}).pipe(take(1));
  }
